...
export default function NewAssessment() {
  ...
  (async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: "user", width: { ideal: 640 }, height: { ideal: 480 }, frameRate: { max: 24 } },
        audio: false
      });
  ...
  <div className="max-w-5xl mx-auto p-6 space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      {/* Left: Live camera + overlay + controls */}
        ...
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded">
          ...
          {countdown !== null && (
            <div className="absolute inset-0 grid place-items-center bg-black/40">
              <div className="text-white text-5xl font-semibold">{countdown}</div>
            </div>
          )}
        </div>

        <div className="mt-3 flex flex-wrap gap-2">
          <button onClick={start} disabled={recording || countdown !== null || busy} className="rounded bg-black text-white px-3 py-2 disabled:opacity-50">Record 10s</button>
          <button onClick={stop} disabled={!recording || busy} className="rounded border px-3 py-2">Stop</button>
          <button onClick={capturePhoto} disabled={recording || countdown !== null || busy} className="rounded border px-3 py-2">Photo</button>
          <button onClick={() => fileRef.current?.click()} disabled={busy} className="rounded border px-3 py-2">Upload</button>
      ...
      <div className="border rounded-lg p-3">
        <div className="aspect-[4/3] w-full overflow-hidden rounded border grid place-items-center bg-gray-50">
          {previewUrl ? (
        ...
        <div className="mt-3 flex items-center gap-2">
          <button onClick={save} disabled={!previewUrl || busy} className="rounded bg-black text-white px-3 py-2 disabled:opacity-50">
            {busy ? "Saving..." : "Save"}
          </button>
          <button onClick={() => { setPreviewUrl(u => { if (u) URL.revokeObjectURL(u); return null; }); setPreviewMime(undefined); }} disabled={!previewUrl || busy} className="rounded border px-3 py-2">
            Clear
          </button>
        </div>
      </div>
